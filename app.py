...
with st.expander("How your data is handled (please read before uploading)"):
    st.markdown(
        ...
        "account number, IFSC code, address, or any other personal identifiers from your statement.\n"
        "- Nothing you upload is saved to a database or shared anywhere.\n"
        "- If you'd rather not upload a real statement, click **Try Demo Data Instead** below to see the "
        "full app in action with sample data."
    )
...
if "pdf_needs_password" not in st.session_state:
...
if df_to_process is not None:
    ...
    if st.session_state["pdf_needs_password"]:
        ...
        entered_password = st.text_input("Enter the PDF password:", type="password", key="pdf_pw_input")
    ...
    with st.spinner("Analyzing your transactions..."):
        try:
            final_df, review_df = process_statement(
                df_to_process,
                password=st.session_state["pdf_password"]
            )
    ...
    st.success(f"Analyzed {len(final_df)} transactions across {len(monthly_summary)} month(s). Your file has been deleted from the server.")
    ...
    fig = px.bar(
        total_by_category, x="category", y="amount", color="category",
    ...
    fig_trend = px.bar(
        monthly_cat_chart, x="month", y="amount", color="category", barmode="group",
    ...
    fig_forecast = px.line(
        forecast_chart_df, x="month", y="savings", color="type", markers=True,
